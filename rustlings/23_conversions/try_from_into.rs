...
pub fn check_range(val: i16) -> bool {
    !(0..=255).contains(&val)
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if check_range(arr[0]) { return Err(IntoColorError::IntConversion); }
        if check_range(arr[1]) { return Err(IntoColorError::IntConversion); }
        if check_range(arr[2]) { return Err(IntoColorError::IntConversion); }
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        if check_range(slice[0]) { return Err(IntoColorError::IntConversion); }
        if check_range(slice[1]) { return Err(IntoColorError::IntConversion); }
        if check_range(slice[2]) { return Err(IntoColorError::IntConversion); }
...
fn main() {
    ...
    println!("range {:?}", check_range(300));
    println!("range {:?}", check_range(200));
    println!("range {:?}", check_range(-300));
    let c1 = Color::try_from((1831, 65, 14));
    println!("hallo {c1:?}");
